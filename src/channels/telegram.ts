...
export class TelegramChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    // Command to clear session (start fresh conversation)
    this.bot.command('clear', (ctx) => {
      ...
      if (cleared) {
        ctx.reply(
          `Session cleared. ${ASSISTANT_NAME} will start fresh on the next message.`,
        );
      } else {
  ...
  async sendDocument(
    ...
    try {
      ...
      await this.bot.api.sendDocument(
        numericId,
        new InputFile(fs.createReadStream(filePath), filePath.split('/').pop()),
